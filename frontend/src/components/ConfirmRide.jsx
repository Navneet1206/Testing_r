...
const ConfirmRide = (props) => {
  ...
  const handleConfirmRide = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/rides/create', rideData, {
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col p-3">
            ...
            <input className="border p-2 rounded w-full" type="date" value={rideDate} onChange={(e) => setRideDate(e.target.value)} required />

            <label className="text-gray-600 text-sm mt-2">Select Ride Time:</label>
            <input className="border p-2 rounded w-full" type="time" value={rideTime} onChange={(e) => setRideTime(e.target.value)} required />
          </div>
        </div>
